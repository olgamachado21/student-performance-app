...
# Nomes amigáveis dos campos, para usar nas mensagens de aviso mostradas ao utilizador.
FIELD_LABELS = {
    "age": "Idade",
...
def check_unusual_values(df: pd.DataFrame, fields: dict) -> list[dict]:
    """
    ...
    for col in NUMERIC_CHECK_FIELDS:
        ...
        # Indica se o valor está acima ou abaixo do normal.
        direction = "acima" if z > 0 else "abaixo"
        warnings.append({
            "field": col,
            "severity": "warning",
            "message": (
                f"{FIELD_LABELS[col]} muito {direction} do habitual "
    ...
    if "G1" in fields and "G2" in fields:
        ...
        if abs(diff) >= GRADE_JUMP_THRESHOLD:
            ...
            warnings.append({
                ...
                "severity": "warning",
                "message": (
                    f"A nota {direction} {abs(diff)} valores entre G1 e G2 — uma mudança grande "
                    "entre dois períodos consecutivos. Confirma que os números estão corretos."
                ),
            })
